...
int saveBitmap(const char *ptrcFileName, int nX, int nY, int nWidth,
               int nHeight);
int loadBitmap(const char *ptrcFileName, GLubyte **bitsp,
...
int loadBitmap(const char *filename, GLubyte **bitsp, BITMAPINFOHEADER **bihp) {
  ...
  // if (fread(&info, 1, infosize, fp) < infosize) { // failed to read in info
  // header
  if (fread(bih, sizeof(BITMAPINFOHEADER), 1, fp) <
      1) { // failed to read in info header
    fclose(fp);
    return 0;
  }

  bitsize = bih->biWidth * bih->biHeight * 3 +
            ((3 * bih->biWidth) % 4 == 0
                 ? 0
                 : (4 - (3 * bih->biWidth) % 4) * bih->biHeight);

  //	int bitsize = nWidth*nHeight*3 +  ((3*nWidth)%4 == 0) ? 0 :
  ...
  if (fread(bits, 1, bitsize, fp) <
      bitsize) { // failed to image data in info header
    free(bih);
    free(bits);
    fclose(fp);
    return 0;
  ...
  // The following reverses ordering of colors,  e.g. (bgr -> rgb), use GL_RGB
  // in glDrawPixels
  /*
  GLubyte temp;
  ...
  */
  ...
  // We can also modify a pixel values at position (x, y), e.g. the following
  // changes the rectangle of size 50 at  (0, 0) to blue color, in case of RGB
  // ordering.
